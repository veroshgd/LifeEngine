"""
Experiment 029 FINAL runner — final_029.py
==========================================

Rehearsal (**burned seeds, full size**):
    python final_029.py --rehearse

Official run (★ allowed only once every pre-launch check has passed ★):
    python final_029.py --final

--------------------------------------------------------------------------------
★★ The sole responsibility of this file: make "running the wrong version" physically impossible ★★
--------------------------------------------------------------------------------------------------
Preregistration: `AI SANDBOX/MEMORY_TRANSFER029_PREREGISTRATION.md` (frozen, see PREREG_SHA256)

```
preflight        frozen constants + module sha256 + task fingerprint + λ + gates
                 + SHUFFLE rule/salt + XSEED donor mapping + preregistration hash
seed guard       --final forces 80000–81499 / N=1500; non-final runs must not touch that block
crash-safe burn  ★ STARTED.lock is created atomically before the **first acquisition trajectory** ★
                 —— 029's seeds start producing information from the acquisition stage,
                    not once the novel task begins. The lock is never deleted automatically.
analysis order   1 acquisition/completeness → 2 G2 capacity → 3 validity verdict
                 → 4 OWN primary → 5 SESOI three-way → 6 SHUFFLE conditional retention
                 → 7 XSEED-DONOR → 8 integrity assertions → 9 secondary → 10 persist
```

⚠ **G1/G3 are preregistered validity gates (before the outcome)**, so they are **enforced** in step 3,
the "validity verdict"; step 8 merely prints the per-seed detail **again**, it is not the first check.
"""
...
REHEARSE_SEED0, REHEARSE_N = 10000, 1500      # burned block, non-zero offset, same size as FINAL
...
RETENTION_MAX = 0.25             # ★ §6.3 criterion B: CI upper bound < 0.25 ★

LEDGER = """0–1499        development (029 probes / calibration / rehearsal use 0–399)
10000–11499   021 holdout set / 028 transport rehearsal / ★029 full-shape rehearsal★
...
# ============================================================ donor mapping
def donor_index(i, n):
    """★ Frozen ★ deterministic half-block rotation. Depends on no memory / outcome."""
    return (i + n // 2) % n


def _assert_donor_mapping(n):
    idx = [donor_index(i, n) for i in range(n)]
    assert all(d != i for i, d in enumerate(idx)), "✗ a self-donor appeared"
    assert sorted(idx) == list(range(n)), "✗ the donor mapping is not a bijection"
    return f"(i+{n // 2})%{n}"
...
def preflight(final):
    ...
    for name, got, want in checks:
        good = got == want
        print(f"  {name:<32}{got!r:<12}{'✓' if good else '✗ expected ' + repr(want)}")
        if not good:
            bad.append(name)
    ...
    if os.path.exists(PREREG_PATH):
        ...
        print(f"  preregistration sha256          {got[:16]}…  "
              f"{'✓' if good else '✗ the preregistration has been changed'}")
        if not good:
            bad.append("preregistration hash")
    else:
        print(f"  preregistration                 ✗ cannot find {PREREG_PATH}")
        if final:
            bad.append("preregistration missing")

    if bad:
        raise SystemExit(f"\n✗ PREFLIGHT failed: {bad}\n"
                         "  A frozen object has been changed. If a change is genuinely needed, an"
                         " AMENDMENT file must be written and the object re-frozen; editing the constant and running is forbidden.")
...
def seed_guard(final, seed0, n):
    ...
    if final:
        if (seed0, n) != (FINAL_SEED0, FINAL_N):
            raise SystemExit(f"✗ --final accepts only seed0={FINAL_SEED0}, N={FINAL_N}")
    else:
        if not (seed0 + n - 1 < lo or seed0 > hi):
            raise SystemExit(f"✗ non-final runs must not touch {lo}–{hi}")
    print(f"\nSeed ledger:\n{LEDGER}")
    print(f"\nThis run: seed0={seed0}  N={n}  "
          f"→ {seed0}–{seed0 + n - 1}   {'★FINAL★' if final else 'rehearsal (burned block)'}")


def create_lock(seed0, n, payload):
    """★ Created atomically and never deleted automatically ★ once created, that seed block is permanently burned."""
    if os.path.exists(RESULT_PATH):
        raise SystemExit(f"✗ {RESULT_PATH} already exists — refusing to re-run")
    try:
        fd = os.open(LOCK_PATH, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
    except FileExistsError:
        raise SystemExit(f"✗ {LOCK_PATH} already exists — this seed block is burned, refusing to re-run")
...
def run_block(seed0, n):
    """★ The lock must already exist before this is called ★ Returns arm → cond → per-seed (C, lat, expo, run)"""
    ...
    for name in ARM_NAMES:
        ...
        for cond in ACQ.CONDITIONS:
            ...
            assert len(rows) == n            # rules 88/91: the whole population, no filtering
            res[name][cond] = rows
...
def joint_bootstrap(darrays):
    """★ One pass, sharing one set of seed indices ★ Returns the bootstrap mean distribution of each array"""
    ...
    for _ in range(N_BOOT):
        ...
        ratio.append(b / a if a > 0 else float("inf"))   # applied per replicate
    for k in out:
        out[k].sort()
    ratio.sort()
    return out, ratio
...
def analyze(seeds, mem, res, trace):
    ...
    p(f"  non-zero evidence rate (not called availability) "
      + "   ".join(f"{c} {nz[c]:.2%}" for c in ACQ.CONDITIONS))
    p("  ★ every primary analysis uses all predefined agents, including incomplete ones and m=0 ★")

    # ---------- 2 G2 capacity transport (group-blind) ----------
    trace.append("2-G2")
    p("\n" + "=" * 78)
    p("2. G2 INTERFACE-CAPACITY TRANSPORT (group-blind: the label is discarded at the input)")
    p("=" * 78)
    pool = sorted(float(mem[(c, sd)].evidence("previously_good_strategy")[0])
                  for c in ACQ.CONDITIONS for sd in seeds)   # ★sorting destroys the grouping★
    states = []
    for sd in seeds:
    ...
    p(f"  G1 acquisition manipulation   {'✓' if g1_ok else '✗'}"
      f"   (bit-identical before the anomaly={same_prefix}, reward opportunity equal)")
    p(f"  G2 interface capacity         {'✓' if g2_ok else '✗'}")
    p(f"  G3 integrity assertions       {'✓' if g3_ok else '✗'}")
    if not g1_ok or not g3_ok:
        p("\n  ✗✗ G1/G3 failed → ★the whole batch is void★ (the developmental history has a leakage path other than memory)")
    if not g2_ok:
        p("\n  ✗ G2 failed, fixed wording:")
        ...
        p("    constraints.")
        p("  ⚠ This is **not** no memory transfer, but calibrated-interface validity")
        p("    compromised. ⛔ λ must not be re-estimated because of it.")

    # ★★ the master switch for confirmatory interpretation ★★
    # If any preregistered validity gate fails → all raw outcomes are still persisted
    # (the FINAL block is already burned, so the data must be kept transparently),
    # but **none** of primary success / functional success / SHUFFLE mediation
    # may be produced any more.
    primary_interpretable = g1_ok and g2_ok and g3_ok
    p(f"\n  ★ confirmatory interpretation: "
      f"{'enabled' if primary_interpretable else '★disabled★ (a validity gate failed)'}")

    # ---------- 4 OWN primary ----------
    trace.append("4-primary")
    p("\n" + "=" * 78)
    p("4. PRIMARY  ΔC = C(Volatile) − C(Stable)  same-seed paired, whole population")
    p("=" * 78)
    ...
    lo, hi = ci(boots["OWN"])
    p("\n" + "=" * 78)
    p(f"5. SESOI three-way reading (SESOI = {SESOI} post-change error)")
    p("=" * 78)
    ...
    # ---------- 6 SHUFFLE conditional retention ----------
    ...
    p("  joint same-seed bootstrap; abs and division applied per replicate")
    if not own_transfer:
        p("  ⚠ " + ("confirmatory interpretation disabled (a validity gate failed)"
                    if not primary_interpretable
                    else "OWN primary did not establish transfer (§6.3.2, the denominator is unidentifiable)")
          + " → **reported descriptively only**,")
        p("    ⛔ no relational mediation is judged.")
    else:
        good = r_hi < RETENTION_MAX
        p(f"  criterion CI upper bound < {RETENTION_MAX}  →  {'met ✓' if good else 'not met ✗'}")
        p("  → " + ("≥75% attenuation established: the transfer is carried by relational structure"
                    if good else
                    "relational structure may not be claimed as the carrier; the contribution of marginal statistics cannot be ruled out"))
    p("  Disclosure: the CI-upper interpretation of §6.3 was finalized after observing the")
    ...
    p("\n" + "=" * 78)
    p("7. XSEED-DONOR (donor mapping (i+N/2)%N, no self-donor, a bijection)")
    p("=" * 78)
    p(f"  ΔC = {dc['XSEED-DONOR']:+.4f}  95% CI [{lo7:+.4f}, {hi7:+.4f}]"
      f"   retains OWN's "
      f"{dc['XSEED-DONOR'] / dc['OWN']:.1%}" if dc["OWN"] else "")

    # ---------- 8 integrity assertion detail ----------
    trace.append("8-integrity")
    p("\n" + "=" * 78)
    p("8. INTEGRITY ASSERTIONS (★assertions, not scientific evidence — rule 93★)")
    p("=" * 78)
    n_del = sum(1 for x in d_del if x == 0)
    n_swp = sum(1 for a, b in zip(d_own, d_swap) if a == -b)
    p(f"  DELETE ΔC ≡ 0: {n_del}/{n} seeds       {'✓' if n_del == n else '✗'}")
    p(f"  SWAP  ΔC ≡ −ΔC(OWN): {n_swp}/{n} seeds  {'✓' if n_swp == n else '✗'}")
    p("  → proves the developmental history has no second path besides memory; under a memory-only architecture this is an algebraic identity.")
    ...
    p("=" * 78)
    p(f"  {'arm':<14}{'Δlatency':>11}{'expo S':>10}{'expo V':>10}"
...
def selftests():
    ...
    try:
        ...
        raise AssertionError("✗ the lock is not atomically exclusive")
    except FileExistsError:
        pass
    os.remove(tmp)
    print("  ✓ lock helper: O_CREAT|O_EXCL atomic exclusivity, a second creation is refused")
    ...
    print("  ✓ worker determinism: acquisition and transfer are reproducible trial by trial")

    # XSEED mapping
    for n in (400, 1500):
        _assert_donor_mapping(n)
    print(f"  ✓ XSEED mapping: no self-donor, a bijection (N=400 and N={FINAL_N})")
    ...
    print("  ✓ SHUFFLE: the permutation is deterministic for a given (seed, n_episodes); the stay/switch counts and"
          " the outcome marginals are unchanged")

    # The three conditions share one donor permutation
    assert donor_index(3, 1500) == donor_index(3, 1500)
    print("  ✓ Stable / Volatile share one donor permutation (the function depends only on i and N)")
...
if __name__ == "__main__":
    ...
    if final == rehearse:
        raise SystemExit("Usage: python final_029.py --rehearse | --final")

    seed0, n = (FINAL_SEED0, FINAL_N) if final else (REHEARSE_SEED0, REHEARSE_N)
    print("=" * 78)
    print(f"029 {'★★ FINAL ★★' if final else 'FULL-SHAPE REHEARSAL (burned block)'}")
    print("=" * 78)
    ...
    if final:
        # ★★ the lock must come before the **first acquisition trajectory** ★★
        create_lock(seed0, n, lock_payload(seed0, n))
        print(f"\n★ {LOCK_PATH} created — {seed0}–{seed0 + n - 1} is permanently burned from now on ★")

    t0 = time.time()
    seeds, mem, res = run_block(seed0, n)
    trace = []
    lines = analyze(seeds, mem, res, trace)
    assert trace == EXPECTED_ORDER, f"✗ the analysis order was disturbed: {trace}"
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("\n".join(header + lines) + "\n")
    print(f"  persisted {os.path.basename(path)}")
    if not final:
        print("\n⚠ This is a rehearsal: not one seed of 80000–81499 was touched, and no FINAL lock was created.")
